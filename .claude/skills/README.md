...
| 스킬 | 하는 일 |
|---|---|
| `/create-issue <설명>` | 설명을 GitHub 이슈로 정리해서 생성 (라벨 · 템플릿 적용) 후 그 이슈의 작업 브랜치로 전환 |
| `/ship-issue [이슈번호 \| 작업 설명] [--no-merge]` | 이슈 번호로 브랜치 생성 → 구현 → 검사 → 커밋 → **보안 체크 · 동작 확인(필수)** → PR → CI 대기 → 머지. 번호 없이 실행하면 이슈부터 생성 |
